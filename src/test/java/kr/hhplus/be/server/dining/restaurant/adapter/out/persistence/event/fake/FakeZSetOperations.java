package kr.hhplus.be.server.dining.restaurant.adapter.out.persistence.event.fake;
...
public class FakeZSetOperations implements ZSetOperations<String, String> {
  ...
  public Double incrementScore(String key, String value, double delta) {
    Map<String, Double> zset = store.computeIfAbsent(key, k -> new HashMap<>());
    double newScore = zset.getOrDefault(value, 0.0) + delta;
  ...
  public Double score(String key, Object value) {
    return store.getOrDefault(key, Map.of()).getOrDefault(value.toString(), null);
  }
